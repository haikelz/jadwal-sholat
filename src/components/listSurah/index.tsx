    ...
    <div className="grid lg:grid-cols-4 grid-cols-2 md:grid-cols-3 grid-rows-1 gap-4">
      {surah.map((surat, index: number) => (
        <Link href={`/quran/surah/${surat.number}`} key={index + 1}>
          <div className="flex flex-col text-left hover:shadow-cards dark:hover:shadow-dark border-2 border-black duration-200 cursor-pointer transition-all p-4 bg-teal-300 dark:bg-[#2A2A37] dark:border-white">
